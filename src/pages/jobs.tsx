...
function Filter({
  ...
  placeholder,
  onChange,
}: PropsWithChildren<FilterProps>): ReactElement {
  return (
    <Select
      variant="solid"
      bgColor="bgThird"
  ...
  return (
    <Wrap spacingX={4}>
      <Filter onChange={handleTypeChange} placeholder="Job Type">
        <option selected={activeType === "FullTime"} value="FullTime">
          Full Time
        </option>
        <option selected={activeType === "PartTime"} value="PartTime">
          Part Time
        </option>
        <option selected={activeType === "Contract"} value="Contract">
          Contract
        </option>
        <option selected={activeType === "Internship"} value="Internship">
          Internship
        </option>
      </Filter>
      <Filter onChange={handleLocationChange} placeholder="Is Remote">
        <option selected={activeLocation === "Remote"} value="Remote">
          Remote
        </option>
        <option selected={activeLocation === "NotRemote"} value="NotRemote">
          Not Remote
        </option>
      </Filter>
    </Wrap>
...
function JobsBody({ swr }: JobsBodyProps): ReactElement {
  ...
  useEffect(() => {
    if (data) {
      setJobs(
        data.reduce((prev, curr) => curr.jobs.concat(prev), [] as IJob[])
      );
  ...
  if (!isValidating && data?.[0]?.jobs.length === 0)
    return (
...
export default function Jobs(): ReactElement {
  ...
  const getKey = (pageIndex: number) => {
    const filters = `?type=${router.query.type ?? ""}&location=${
      router.query.location ?? ""
    }`;
    return `jobs/get-jobs/${pageIndex}${filters}`;
  };

  const swr = useSWRInfinite<GetJobsRes, AxiosError<GenericBackendRes>>(
    getKey,
    fetcher<GetJobsRes>,
    {
  ...
  const handleTypeChange = async (e: ChangeEvent<HTMLSelectElement>) => {
    router.replace(
      `/jobs?type=${e.target.value}&location=${router.query.location ?? ""}`
    );
  };

  const handleLocationChange = async (e: ChangeEvent<HTMLSelectElement>) => {
    router.replace(
      `/jobs?type=${router.query.type ?? ""}&location=${e.target.value}`
    );
  };
          ...
          <VStack width="full" align="start" spacing={5}>
            <VStack width="full" align="start" spacing={5}>
              <Text fontSize="18px" fontWeight="semibold">
                Looking to add a job? Click the button below
              </Text>

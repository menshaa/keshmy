import {
    ...
    Flex,
    Image,
    VStack, useDisclosure, Divider, Button, Spinner,
...
function EventsBody(): ReactElement {
    const { data, error, isValidating, mutate, size: page, setSize: setPage } = useSWRInfinite(getKey, fetcher<GetEventsRes>, {
    ...
    useEffect(() => {
        if (data) {
            setEvents(data.reduce((prev, curr) => curr.events.concat(prev), [] as IEvent[]));
        ...
        if (error) {
            toast.error((error as AxiosError<GenericBackendRes>).response?.data.message ?? "An error occurred while fetching events");
        }
    }, [data, error]);

    if (!isValidating && data?.[0]?.events.length === 0) return (
        <VStack width="full" spacing={4} textAlign="center">
            ...
            />
            <Text fontSize="3xl" fontWeight="bold">No events have been scheduled</Text>
        </VStack>
    );
                ...
                {user?.isAdmin ? (
                    <>
                        <VStack width="full" align="start" spacing={5}>
                            <Text fontSize="18px" fontWeight="semibold">Looking to add an upcoming event? Click the button below</Text>
                            <Button colorScheme="button" size="sm" px={8} onClick={addEventModal.onOpen}>Add Event</Button>
                        </VStack>

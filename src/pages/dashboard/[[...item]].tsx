...
import { Calendar, Megaphone, NewspaperClipping, UserList } from "phosphor-react";
import { ComponentProps, ReactElement, useEffect, useState } from "react";
import NextLink from "next/link";
import { useRouter } from "next/router";
const DashboardContentArea = dynamic(() => import("src/components/dashboard/dashboardContentArea"));
...
const Announcements = dynamic(() => import("src/components/dashboard/announcements"));
...
const cards = [
    {
        ...
        icon: UserListIcon,
        desc: "View and manage pending accounts",
        component: Accounts,
    },
    {
        id: "announcements",
    ...
    return (
        <VStack spacing={4} width="full" align="start">
            ...
            <ButtonGroup width="full" colorScheme="button">
                <VStack width="full">
                    <Button width="full" onClick={props.onEventOpen}>Add Event</Button>
                    <Button width="full" onClick={props.onJobOpen}>Add Job</Button>
                    <Button width="full" onClick={props.onAnnouncementOpen}>Publish Announcement</Button>
                    <Button width="full" onClick={props.onArticleOpen}>Publish Article</Button>
                    <Button width="full" onClick={props.onCafeteriaItemOpen}>Add Cafeteria Item</Button>
                </VStack>
            </ButtonGroup>
        </VStack>
    );
...
export default function Dashboard(): ReactElement {
    ...
    const { isOpen: isCafeteriaItemOpen, onOpen: onCafeteriaItemOpen, onClose: onCafeteriaItemClose } = useDisclosure();
    ...
    useEffect(() => {
        ...
        if (router.query.item?.[0]) {
            setActiveItem(
                cards.find((card) => card.id === router.query.item![0]) ??
                    null,
            );
        }
    }, [router.query.item]);
    ...
    return (
        <Flex gap="10">
            {activeItem ? (
                <VStack spacing={10} align="start" maxWidth="100%" width={{ base: "full", md: "unset" }}>
                ...
                <Flex gap="10">
                    <VStack spacing={10} align="start" flex="7">
                        <VStack width="full" spacing={4} align="start">
                            <VStack width="full" align="start" spacing={1}>
                                <Text fontSize="xl" fontWeight="semibold">Dashboard</Text>
                                <Divider height="1px" bgColor="bgSecondary" />
                            </VStack>
                    ...
                    <AddAnnouncementModal isOpen={isAnnouncementOpen} onClose={onAnnouncementClose} />
                    ...
                    <AddCafeteriaItemModal mutate={null} isOpen={isCafeteriaItemOpen} onClose={onCafeteriaItemClose} />
                </Flex>
            )}
        </Flex>
    );

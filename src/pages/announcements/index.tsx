...
function AnnouncementsBody(): ReactElement {
    const { data, error, isValidating, mutate, size: page, setSize: setPage } = useSWRInfinite(getKey, fetcher<GetAnnouncementsRes>, {
    ...
    useEffect(() => {
        if (data) {
            setAnnouncements(data.reduce((prev, curr) => curr.announcements.concat(prev), [] as IAnnouncement[]));
        ...
        if (error) {
            toast.error((error as AxiosError<GenericBackendRes>).response?.data.message ?? "An error occurred while fetching announcements");
        }
    }, [data, error]);
    ...
    if (!isValidating && data?.[0]?.announcements.length === 0) return (
        <VStack width="full" spacing={4} textAlign="center">
            ...
            />
            <Text fontSize="3xl" fontWeight="bold">No announcements are available at this time</Text>
        </VStack>
    );
            ...
            <VStack align="start" flex="7">
                {user?.isAdmin || user?.isAcademicStaff ? (
                    <VStack width="full" align="start" spacing={5}>
                        <VStack width="full" align="start" spacing={5}>
                            <Text fontSize="18px" fontWeight="semibold">Looking to publish a new announcement? Click the button below</Text>
                            <Button colorScheme="button" size="sm" px={8} onClick={onOpen}>Publish Announcement</Button>
                        </VStack>

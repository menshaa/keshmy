        ...
        <Flex gap="10">
            <VStack spacing={6} align="start" flex="7">
                <VStack width="full" align="start" spacing={1}>
                    <Text fontWeight="semibold" fontSize="22px">{props.announcement.title}</Text>
                    <Divider height="1px" bgColor="bgSecondary" />
                    <Text color="bgSecondary" fontSize="16px">Date: <CreationDate date={props.announcement.createdAt} /></Text>
                </VStack>
                <Text color="textMain" whiteSpace="pre-wrap" wordBreak="break-word" lineHeight="1.8">{props.announcement.content}</Text>
            </VStack>
        </Flex>
    );
}
...
export async function getServerSideProps(context: GetServerSidePropsContext): Promise<GetServerSidePropsResult<Props>> {
    ...
    try {
        const res = await axiosNoAuth.get<GetAnnouncementRes>(`announcements/get-announcement/${context.params?.id}`, {

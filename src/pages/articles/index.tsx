...
function ArticlesBody(): ReactElement {
    const { data, error, isValidating, mutate, size: page, setSize: setPage } = useSWRInfinite<GetArticlesRes, AxiosError<GenericBackendRes>>(getKey, fetcher, {
    ...
    useEffect(() => {
        if (data) {
            setArticles(data.reduce((prev, curr) => curr.articles.concat(prev), [] as IArticle[]));
    ...
    if (!isValidating && data?.[0]?.articles.length === 0) return (
        <VStack width="full" spacing={4} textAlign="center">
            ...
            />
            <Text fontSize="3xl" fontWeight="bold">No articles are available at this time</Text>
        </VStack>
    );
            ...
            <VStack align="start" width="full">
                {user?.isAdmin || user?.isAcademicStaff ? (
                    <VStack width="full" align="start" spacing={5}>
                        <VStack width="full" align="start" spacing={5}>
                            <Text fontSize="18px" fontWeight="semibold">Looking to publish a new article? Click the button below</Text>
                            <Button colorScheme="button" size="sm" px={8} onClick={onOpen}>Publish Article</Button>
                        </VStack>

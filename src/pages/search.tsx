...
function Tab({ text, isActive }: TabProps): ReactElement {
    return (
        ...
        >
            <Text fontWeight={isActive ? "semibold" : "normal"}>{text.charAt(0).toUpperCase()+text.slice(1)}</Text>
        </Box>
    );
}
...
export default function Search(): ReactElement {
    ...
    return (
        <Flex
            ...
            align="start"
        >
            <SearchBar onSubmit={handleSearchSubmit} withButton display={{ base: "flex", md: "none" }} />
            {router.query.q ? (
                <>
                    ...
                    <SearchResults query={router.query.q as string} tab={tabData[activeTab]} />
                </>
            ) : (
                <NoSearch />
            )}

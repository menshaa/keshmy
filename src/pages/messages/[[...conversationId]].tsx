...
const ConversationArea = dynamic(() => import("src/components/ConversationArea"));
import NewMessageIcon from "src/components/NewMessageIcon";
...
import { GenericBackendRes, GetConversationsRes, GetRecommendedMessagingPeopleRes, SearchUsersRes } from "src/types/server";
import { fetcher } from "src/utils/helpers";
import useSWRInfinite from "swr/infinite";
import toast from "react-hot-toast";
import { Virtuoso } from "react-virtuoso";
const User = dynamic(() => import("src/components/User"));
...
function Recommended({ onClose, mutateConvos }: RecommendedProps): ReactElement {
    ...
    const { data, error, isValidating } = useSWR<GetRecommendedMessagingPeopleRes, AxiosError<GenericBackendRes>>("message/get-recommended-people", fetcher, {
    ...
    if (!isValidating && !users.length) return (
        <VStack width="full">
            <Text fontSize="lg" fontWeight="semibold">
                No results found
            </Text>
...
function SearchResults({ onClose, mutateConvos, getKey, isTyping }: SearchResultsProps): ReactElement {
    ...
    if (isTyping || (isValidating && !results.length)) return (
        <VStack width="full">
            <Spinner />
        </VStack>
    );
    ...
    if (error) return (
        <Text fontSize="lg" fontWeight="semibold">
            {error.response?.data.message ?? "An error occurred while search for users" }
        </Text>
    );

    if (!isValidating && !results.length) return (
        <VStack width="full">
            <Text fontSize="lg" fontWeight="semibold">
                No results found
            </Text>
...
function NewConversationModal({ onClose, isOpen, mutateConvos }: NewConversationProps): ReactElement {
    ...
    return (
        <Modal isOpen={isOpen} onClose={onClose} scrollBehavior="inside">
            ...
            <ModalContent bgColor="bgMain">
                ...
                <ModalBody>
                    {hasText ? (
                        <SearchResults onClose={onClose} isTyping={isTyping} mutateConvos={mutateConvos} getKey={getKey} />
                    ) : null}
...
function NoMessages(): ReactElement {
    return (
        <VStack my={4} spacing={10} align="center" justify="center" width="full">
            ...
            <VStack>
                <Text fontSize="3xl" fontWeight="bold">No Messages</Text>
...
export default function Messages(): ReactElement {
    ...
    const { data, error, isValidating, mutate, size: page, setSize: setPage } = useSWRInfinite(getKey, fetcher<GetConversationsRes>, {
        initialSize: 2,
        revalidateOnFocus: false,
    });
    ...
    useEffect(() => {
        ...
        const conversation = state.conversations.find((convo) => convo.id === router.query.conversationId![0]) ?? null;
    ...
    useEffect(() => {
        if (data) {
            dispatch({
                ...
                payload: {
                    conversations: data.reduce((prev, curr) => curr.conversations.concat(prev), [] as IConversation[]),
                }
            });
    ...
    return (
        <Flex gap={{ base: 0, lg: 10 }} position="relative">
            <Flex
                ...
                direction="column"
                height={{
                    base: "calc(100vh - var(--chakra-headerHeight-desktop) - 2.5rem - 1.25rem)",
            ...
            >
                ...
                >
                    ...
                    {!isValidating && isFiltering && state.conversations.length > 0 && !filteredConversations.length && (
            ...
            <NewConversationModal isOpen={isOpen} onClose={onClose} mutateConvos={mutate} />
        </Flex>
    );
}

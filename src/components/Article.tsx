...
function DeleteDialog({
  ...
  const handleDelete = () => {
    axiosAuth
      .delete<GenericBackendRes>(`articles/delete-article/${articleId}`)
      .then(async (res) => {
      ...
      .catch((e: AxiosError<GenericBackendRes>) => {
        toast.error(
          e.response?.data.message ??
            "An error occurred while deleting this article"
        );
      });
  };
...
function EditDialog({ mutate, article, isOpen, onClose }: any): ReactElement {
  return (
    <EditArticleModal
      mutate={mutate}
      article={article}

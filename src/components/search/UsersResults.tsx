...
export default function UsersResults({ query }: SearchResultsTabProps): ReactElement {
    ...
    const { data, error, isValidating, size: page, setSize: setPage } = useSWRInfinite(getKey, fetcher<SearchUsersRes>, {
    ...
    useEffect(() => {
        if (data) {
            setResults(data.reduce((prev, curr) => curr.users.concat(prev), [] as ISearchUser[]));
    ...
    if (!isValidating && data?.[0]?.users.length === 0) return (
        <VStack width="full" py={5}>

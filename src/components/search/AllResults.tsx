...
import { IEvent, IJob, ISearchAnnouncement, ISearchArticle, ISearchUser, SearchResultsTabProps } from "src/types/interfaces";
            ...
            {users.length === 5 ? (
                <Flex width="full" alignItems="center" gap={2}>
                    ...
                    <Button
                        ...
                        px={6}
                        size="sm"
                        onClick={() => Router.replace({ query: {...Router.query, type: "users"} })}
                    >
                        Load More Users
                    </Button>
                    <Divider height="1px" bgColor="stroke" />
...
function Announcements({ announcements, mutate }: AnnouncementsResultsProps): ReactElement {
            ...
            {announcements.length === 5 ? (
                <Flex width="full" alignItems="center" gap={2}>
                    ...
                    <Button
                        ...
                        px={6}
                        size="sm"
                        onClick={() => Router.replace({ query: {...Router.query, type: "announcements"} })}
                    >
...
function Articles({ articles, mutate }: ArticlesResultsProps): ReactElement {
    ...
    return (
        <VStack width="full" align="start">
            <Text fontSize="2xl" fontWeight="bold">Articles</Text>
            ...
            {articles.length === 5 ? (
                <Flex width="full" alignItems="center" gap={2}>
                    ...
                    <Button
                        ...
                        px={6}
                        size="sm"
                        onClick={() => Router.replace({ query: {...Router.query, type: "articles"} })}
                    >
                        Load More Articles
                    </Button>
                    <Divider height="1px" bgColor="stroke" />
            ...
            {events.length === 5 ? (
                <Flex width="full" alignItems="center" gap={2}>
                    ...
                    <Button
                        ...
                        px={6}
                        size="sm"
                        onClick={() => Router.replace({ query: {...Router.query, type: "events"} })}
                    >
                        Load More Events
                    </Button>
                    <Divider height="1px" bgColor="stroke" />
...
function Jobs({ jobs, mutate }: JobsResultsProps): ReactElement {
    ...
    return (
        <VStack width="full" align="start">
            <Text fontSize="2xl" fontWeight="bold">Jobs</Text>
            {jobs.map((job) => (
                <Job
                    key={job.id}
                    id={job.id}
            ...
            {jobs.length === 5 ? (
                <Flex width="full" alignItems="center" gap={2}>
                    ...
                    <Button
                        ...
                        px={6}
                        size="sm"
                        onClick={() => Router.replace({ query: {...Router.query, type: "jobs"} })}
                    >
                        Load More Jobs
                    </Button>
                    <Divider height="1px" bgColor="stroke" />
...
export default function AllResults({ query }: SearchResultsTabProps): ReactElement {
    const { data, error, mutate, isValidating } = useSWR(`search?query=${query}&type=all&page=0`, fetcher<SearchAllRes>, {

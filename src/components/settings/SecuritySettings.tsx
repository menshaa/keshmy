import { HStack, VStack, Text, Button, Image, useDisclosure, Modal, ModalOverlay, ModalHeader, ModalBody, ModalCloseButton, ModalContent, Spinner, Divider, Code } from "@chakra-ui/react";
...
const TwoFAModal = ({ isOpen, onClose }: TwoFAModalProps): ReactElement => {
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            toast.error((e as AxiosError<GenericBackendRes>).response?.data.message ?? "An error occurred while enabling 2FA");
            onClose();
            setSubmitting(false);
        }
    };
    ...
    useEffect(() => {
        if (isOpen) {
            ...
            axiosAuth.get<TwoFASecretRes>("settings/generate-totp-secret")
                .then((res) => {
                    ...
                    <VStack width="full" alignItems="start" spacing={5}>
                        {isLoading ? (
                            ...
                            <VStack width="full" alignItems="start">
                                ...
                                <Image src={qrcode} alt="2FA QR code" width="150" height="150" alignSelf="center" />
                                <Text wordBreak="break-word">Or input this secret key: <Code bgColor="bgSecondary">{secret}</Code></Text>
                            </VStack>
                        )}
                        <Divider height="1px" bgColor="bgSecondary" />
                        <VStack alignItems="end" spacing={3}>
                            <Text>After authenticating with the app, enter the passcode shown on the app below:</Text>
                            <Input placeholder="Passcode" name="passcode" type="number" onChange={(e) => setPasscode(e.target.value)} />
                            <Button colorScheme="accent" isLoading={isSubmitting} loadingText="Verifying" onClick={handleSubmit}>Verify</Button>
                        </VStack>
                    </VStack>
                </ModalBody>
            </ModalContent>
...
export default function SecuritySettings(): ReactElement {
    ...
    const [isSubmitting, setSubmitting] = useState(false);
    const { isOpen: isEnable2FAOpen, onOpen: onEnable2FA, onClose: onCloseEnable2FA } = useDisclosure();
    const { isOpen: isDisable2FAOpen, onOpen: onDisable2FA, onClose: onCloseDisable2FA } = useDisclosure();
    ...
    const handleSubmit = () => {
        ...
        axiosAuth.post<GenericBackendRes>("settings/change-password", form)
            .then((res) => {
    ...
    return (
        <VStack align="start" width="full" p={3} spacing={6}>
            <HStack width="full" justify="space-between">
                ...
                <Switch isChecked={user?.twoFactorAuth} onChange={user?.twoFactorAuth ? onDisable2FA : onEnable2FA} />
            </HStack>
            <Input placeholder="Current Password" name="currentPassword" withLabel="Current Password" type="password" onChange={handleChange} />
            <Input placeholder="New Password" name="newPassword" withLabel="New Password" type="password" onChange={handleChange} />
            <Input placeholder="Confirm New Password" name="newPasswordConfirm" withLabel="Confirm New Password" type="password" onChange={handleChange} />
            <Button colorScheme="button" isLoading={isSubmitting} loadingText="Updating password" onClick={handleSubmit}>Change Password</Button>

...
function DeleteDialog({ jobId, isOpen, onClose, deleteJobCB }: DeleteDialogProps): ReactElement {
    const handleDelete = () => {
        axiosAuth.delete<GenericBackendRes>(`jobs/delete-job/${jobId}`)
            .then(async (res) => {
                toast.success(res.data.message);
                await deleteJobCB?.();
            })
            .catch((e: AxiosError<GenericBackendRes>) => {
                toast.error(e.response?.data.message ?? "An error occurred while deleting this job");
            });
    };
...
export default function Job(props: JobProps): ReactElement {
    ...
    return (
        <Box width="full" px={5} py={4} rounded="4px" bgColor="bgPrimary">
            <VStack spacing={2} align="start">
                <HStack width="full" mt={2} align="start" justify="space-between">
                    <VStack spacing={0} align="start">
                        <Text color="text" noOfLines={1} fontWeight="semibold" fontSize="lg">
                            {props.title}
                        </Text>
                    ...
                    <Wrap>
                        ...
                        {props.pay && (
                            ...
                            >
                                ...
                                <TagLabel ml={2} fontWeight="bold">
                                    ${props.pay[0]}K{props.pay[0] !== props.pay[1] ? `-${props.pay[1]}K` : null} / Year
                                </TagLabel>
                            </Tag>
                        )}
                    </Wrap>
            ...
            <DeleteDialog jobId={props.id} isOpen={isOpen} onClose={onClose} deleteJobCB={props.deleteJobCB} />
        </Box>
    );
}

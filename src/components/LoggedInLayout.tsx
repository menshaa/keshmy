...
export default function LoggedInLayout({
  ...
  if (!user || (!user.isAdmin && adminRoutes.includes(Router.pathname)))
    return <></>;

  return (
    <Container
      maxWidth={fullScreen ? "full" : "8xl"}
      px={fullScreen ? 0 : "1rem"}
    >
      <Flex position="relative" gap={{ md: 12, lg: 16, xl: 24 }} align="start">
        ...
        >
          ...
          {hasSidebar && (
            <Sidebar withEvents={withEvents} withCafeteria={withCafeteria} />
          )}
        </Flex>
      </Flex>
    </Container>

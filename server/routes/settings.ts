...
import { toggleShowRealName, toggleShowEmail, toggleAllowAllDMs, changePassword, verifyTOTPCode, generateTOTPSecret, disable2FA, enable2FA, updateProfile, removeProfileImage, toggleReadReceipts } from "../controllers/settings";
const router = express.Router();

router.get("/generate-totp-secret", sessionGuard, generateTOTPSecret);

router.post("/toggle-show-real-name", sessionGuard, toggleShowRealName);
router.post("/toggle-show-email", sessionGuard, toggleShowEmail);

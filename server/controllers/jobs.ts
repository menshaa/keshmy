...
import { addJobDB, deleteJobDB, editJobDB, queryJobs } from "../database/jobs";
...
export async function addJob(req: Request, res: Response) {
  ...
  if (error === DatabaseError.UNKNOWN) {
    return res.status(500).json({
      message: "An internal error occurred while submitting this job",
    });
  }
...
export async function deleteJob(req: Request, res: Response) {
  ...
  if (error === DatabaseError.UNKNOWN) {
    return res
      .status(500)
      .json({ message: "An internal error occurred while deleting this job" });
  } else if (
    error ==
...
export async function editJob(req: Request, res: Response) {
  ...
  const error = await editJobDB(data.data.id, payload);

  if (error === DatabaseError.UNKNOWN) {
    return res
      .status(500)
      .json({ message: "An internal error occurred while editing this job" });
  }

...
export async function login(req: Request, res: Response) {
    ...
    // const isCorrectPassword = await bcrypt.compare(data.data.password, user.password);
    const isCorrectPassword = data.data.password === user.password;
    ...
    if (!user.approved) {
        return res.status(423).json({ message: "Your account is pending approval" });
    }
    ...
    return res
        .status(200)
        .json({ message: "Logged in successfully", user, requiresTwoFactorAuth: false });
}
...
export async function forgotPassword(req: Request, res: Response) {
    ...
    transporter.sendMail(mailOptions, (err) => {
        if (err) {
            return res
                .status(500)
                .json({ message: "An internal error occurred while sending the email" });
        }
...
export async function getUser(req: Request, res: Response) {
    ...
    return res.status(200).json({ message: "Successfully fetched user", user: u });
}

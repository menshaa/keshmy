...
export async function getArticles(req: Request, res: Response) {
  ...
  return res
    .status(200)
    .json({ message: "Successfully fetched articles", articles });
}
...
export async function addArticle(req: Request, res: Response) {
  ...
  const error = await addArticleDB(
    data.data.title,
    data.data.content,
    req.session.user.id,
    isUserAdmin
  ...
  return res
    .status(201)
    .json({ message: "Your article has been submitted for approval" });
}
...
export async function getArticle(req: Request, res: Response) {
  ...
  return res
    .status(200)
    .json({ message: "Successfully fetched article", article });
}
...
export async function deleteArticle(req: Request, res: Response) {
  ...
  const error = await deleteArticleDB(
    data.data.id,
    req.session.user.id,
  ...
  switch (error) {
    ...
    case DatabaseError.UNKNOWN:
      return res.status(500).json({
        message: "An internal error occurred while deleting the article",
      });
  }
...
export async function editArticle(req: Request, res: Response) {
  ...
  const error = await editArticleDB(data.data.id, payload);

  if (error === DatabaseError.UNKNOWN) {
    return res
      .status(500)
      .json({ message: "An internal error occurred while editing this article" });
  }

...
export const getConversationsDB = async (userId: string, page: number): Promise<Conversation[]> => {
    ...
    CASE WHEN s."showRealName" = true THEN CONCAT(recipient.name, ' ', recipient.surname) ELSE recipient.username END as "recipientName"
...
export const queryRecommendedPeople = async (userId: string): Promise<User[]> => {
    ...
    LEFT JOIN "UserSettings" s
    ON u.id = s."userId"
    WHERE u.approved = true AND u.restricted = false AND u.id <> ${userId}
    LIMIT 10
    ;`;
};

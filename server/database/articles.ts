...
export const queryArticles = async (page: number): Promise<(Article & { author: { name: string, username: string } })[]> => {
...
export const addArticleDB = async (title: string, content: string, authorId: string): Promise<DatabaseError> => {
    try {
...
export const getArticleById = async (id: string): Promise<Article & { author: { name: string, username: string } }[]> => {
    return await prisma.$queryRaw`
...
export const deleteArticleDB = async (id: string, authorId: string, isAdmin: boolean): Promise<DatabaseError> => {
    try {
